...
correct_pre = tf.equal(tf.arg_max(expec_outputs, 1), tf.arg_max(outputs, 1))
...
for i in range(100):
    ...
    test = data.test_set(20)
    train_step.run(feed_dict={img_inputs: batch[0], expec_outputs: batch[1]}, session=sess)
    train_acc = accuracy.eval(feed_dict={img_inputs: test[0], expec_outputs: test[1]}, session=sess)
